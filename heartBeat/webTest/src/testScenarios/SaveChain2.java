...
/**
 * User: luc  | Date: 8/12/13  |  Time: 11:37 PM
 */
public class SaveChain2 {
    //Node 1
    ...
    public static void main(String[] args) {
        String filePath = "/home/luc/Downloads/nvidia.run";
        String userPath = "/normal/user2";

        FileInfo fileInfo = null;
        try {
            fileInfo = new FileInfo("nvidia.run", userPath, FileUtils.getFileChecksum(new File(filePath)), new File(filePath).length(), 2);
            ...
            SendData<ChainInfo> sender = new SendData<ChainInfo>(node1Ip, node1Port, HeaderMessage.SAVE_CHAIN, chain, new File(filePath));
            sender.send();
        } catch (IOException e) {
